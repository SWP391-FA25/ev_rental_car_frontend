    ...
    async function fetchStats() {
      ...
      try {
        ...
        const todayRevenue = bookings
          .filter(b => {
            const completedDate = b.actualEndDate ? new Date(b.actualEndDate) : null;
            return completedDate && completedDate >= today && (b.status || b.bookingStatus) === 'COMPLETED';
          })
          .reduce((sum, b) => {
            const base = b.basePrice || 0;
            const insurance = b.insuranceAmount || 0;
        ...
        const weekRevenue = bookings
          .filter(b => {
            const completedDate = b.actualEndDate ? new Date(b.actualEndDate) : null;
            return completedDate && completedDate >= weekAgo && (b.status || b.bookingStatus) === 'COMPLETED';
          })
          .reduce((sum, b) => {
            const base = b.basePrice || 0;
            const insurance = b.insuranceAmount || 0;
        ...
        const monthRevenue = bookings
          .filter(b => {
            const completedDate = b.actualEndDate ? new Date(b.actualEndDate) : null;
            return completedDate && completedDate >= monthStart && (b.status || b.bookingStatus) === 'COMPLETED';
          })
          .reduce((sum, b) => {
            const base = b.basePrice || 0;
            const insurance = b.insuranceAmount || 0;
        ...
        const lastMonthRevenue = bookings
          .filter(b => {
            const completedDate = b.actualEndDate ? new Date(b.actualEndDate) : null;
            return completedDate && completedDate >= lastMonthStart && completedDate <= lastMonthEnd && (b.status || b.bookingStatus) === 'COMPLETED';
          })
          .reduce((sum, b) => {
            const base = b.basePrice || 0;
            const insurance = b.insuranceAmount || 0;
        ...
        const growthBookings = lastMonthCount === 0 ? (currentMonthCount > 0 ? 100 : 0) : Math.round(((currentMonthCount - lastMonthCount) / lastMonthCount) * 100);

        // Tính tăng trưởng Active Bookings
        const currentMonthActive = bookings.filter(b => {
          const d = new Date(b.createdAt);
          return (b.status || b.bookingStatus) === 'IN_PROGRESS' && d.getMonth() === currentMonth && d.getFullYear() === currentYear;
        }).length;
        const lastMonthActive = bookings.filter(b => {
          const d = new Date(b.createdAt);
          return (b.status || b.bookingStatus) === 'IN_PROGRESS' && d.getMonth() === lastMonth && d.getFullYear() === lastMonthYear;
        }).length;
        const growthActive = lastMonthActive === 0 ? (currentMonthActive > 0 ? 100 : 0) : Math.round(((currentMonthActive - lastMonthActive) / lastMonthActive) * 100);
        ...
        const currentMonthCompleted = bookings.filter(b => {
          ...
          return (b.status || b.bookingStatus) === 'COMPLETED' && d.getMonth() === currentMonth && d.getFullYear() === currentYear;
        }).length;
        const lastMonthCompleted = bookings.filter(b => {
          const d = new Date(b.actualEndDate || b.endTime || b.createdAt);
          return (b.status || b.bookingStatus) === 'COMPLETED' && d.getMonth() === lastMonth && d.getFullYear() === lastMonthYear;
        }).length;
        const growthCompleted = lastMonthCompleted === 0 ? (currentMonthCompleted > 0 ? 100 : 0) : Math.round(((currentMonthCompleted - lastMonthCompleted) / lastMonthCompleted) * 100);
        ...
        bookings
          .filter(b => (b.status || b.bookingStatus) === 'COMPLETED' && (b.actualEndDate || b.endTime || b.createdAt))
          .forEach(b => {
            let dateObj = null;
    ...
    <div className="w-full min-h-screen p-8 transition-colors duration-300" style={{ background: 'var(--background)', color: 'var(--foreground)' }}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          ...
          <div className="flex items-start justify-between mb-3">
            <div>
              ...
              <p className="text-2xl font-bold" style={{ color: 'var(--foreground)' }}>
                {loadingStats ? '...' : `${stats.totalRevenue.toLocaleString('vi-VN')} ₫`}
              </p>
            </div>
        ...
        <div className="p-6 rounded-xl shadow-sm border" style={{ background: 'var(--card)', color: 'var(--card-foreground)', borderColor: 'var(--border)' }}>
          ...
          <p className="text-xs mt-1" style={{ color: 'var(--muted-foreground)' }}>Success rate: {stats.totalBookings > 0 ? Math.round((stats.completedBookings / stats.totalBookings) * 100) : 0}%</p>
        </div>
      </div>
