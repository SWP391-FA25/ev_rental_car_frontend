...
export default function CarRentalContract({ bookingId, onStatusChange }) {
  ...
  const loadUserBookings = async () => {
    try {
      ...
      if (response.success) {
        ...
        const bookingWithContract = response.data.bookings?.find(
          b => b.hasContract
        );
  ...
  const loadContract = async () => {
    try {
      ...
      const response = await apiClient.get(
        `/api/bookings/${selectedBookingId}/contract`
      );
      if (response.success) {
  ...
  const handleDigitalSign = async () => {
    ...
    if (!activeBookingId) {
      toast.error(
        tr('contract.messages.noBookingSelected', 'Chưa chọn đặt xe')
      );
      return;
    }
    if (
    ...
    return (
      <div className='text-center py-8'>
        ...
        <p className='text-muted-foreground'>
          {tr('contract.messages.noBookings', 'Bạn chưa có đặt xe nào')}
        </p>
      </div>
    );
  }
    ...
    return (
      <div className='flex items-center justify-center h-64'>
        <div className='text-lg'>
          {tr('contract.messages.loading', 'Đang tải...')}
        </div>
      </div>
    );
  }
            ...
            >
              <CardHeader>
                ...
                <div className='flex items-center space-x-2'>
                  <Badge
                    variant={booking.hasContract ? 'default' : 'secondary'}
                  >
                    {booking.hasContract
                      ? t('contract.status.hasContract')
                      : t('contract.status.noContract')}
                  </Badge>
                </div>
              </CardHeader>
              <CardContent>
  ...
  if (!contract && selectedBookingId) {
    return (
      <div className='text-center py-8'>
        <FileTextIcon className='mx-auto h-12 w-12 text-muted-foreground mb-4' />
        <p className='text-muted-foreground'>
          {tr('contract.messages.noContract', 'Không tìm thấy hợp đồng')}
        </p>
        {!bookingId && (
          <Button
            variant='outline'
  ...
  return (
    <div className='space-y-6'>
      <div>
        <h1 className='text-3xl font-bold tracking-tight'>Hợp đồng thuê xe</h1>
        <p className='text-muted-foreground'>
