...
export default function ReturnCar() {
  ...
  const handleSelectBooking = async value => {
    ...
    try {
      ...
      try {
        ...
        if (vehId) {
          const inspRes = await apiClient.get(endpoints.inspections.getByVehicle(vehId));
          const data = inspRes?.data;
          const list = Array.isArray(data?.inspections) ? data.inspections : Array.isArray(data) ? data : Array.isArray(data?.items) ? data.items : [];
          const sorted = [...list].sort((a, c) => new Date(c?.createdAt || 0) - new Date(a?.createdAt || 0));
          const latest = sorted.find(i => typeof i?.mileage === 'number' || typeof i?.odometer === 'number');
          const latestOdo = typeof latest?.mileage === 'number' ? latest.mileage : (typeof latest?.odometer === 'number' ? latest.odometer : null);
          ...
          setLastRecordedOdometer(typeof b?.pickupOdometer === 'number' ? b.pickupOdometer : null);
        }
      } catch {
        setLastRecordedOdometer(typeof b?.pickupOdometer === 'number' ? b.pickupOdometer : null);
      }
  ...
  const getOdoError = (value, currentBooking) => {
    ...
    const baseMin = typeof lastRecordedOdometer === 'number'
      ? lastRecordedOdometer
      : (Number.isFinite(pickup) ? pickup : undefined);
  
    if (typeof baseMin === 'number') {
      if (odo < baseMin) {
        return t('staffReturnCar.toast.invalidReturnOdometerMin', { pickup: baseMin });
      }
      const diffBase = Number.isFinite(pickup) ? pickup : baseMin;
      if (odo - diffBase > MAX_ODOMETER_DIFF) {
        return t('staffReturnCar.toast.invalidReturnOdometerRange', { max: MAX_ODOMETER_DIFF });
      }
    }
    return '';
  };
  ...
  const handleCompleteReturn = async () => {
    ...
    // Additional validation per booking.middleware: >= pickup và chênh lệch <= MAX_ODOMETER_DIFF
    const pickup = Number(booking?.pickupOdometer);
    if (Number.isFinite(pickup)) {
      if (odo < pickup) {
        toast.error(t('staffReturnCar.toast.invalidReturnOdometerMin', { pickup }));
        return;
      }
      if (odo - pickup > MAX_ODOMETER_DIFF) {
        toast.error(t('staffReturnCar.toast.invalidReturnOdometerRange', { max: MAX_ODOMETER_DIFF }));
        return;
      }
    }
      ...
      if (hasIncident && incidentFiles.length === 0) {
        ...
        toast.error(t('staffReturnCar.incident.imagesRequired'));
        return;
      }
      ...
      try {
        ...
        try {
          ...
          const incidentImageUrls =
            hasIncident && incidentFiles.length
          ...
          const inspectionPayload = {
            ...
            images:
              hasIncident && incidentImageUrls.length
                ? incidentImageUrls
                : undefined,
          ...
          <CardContent className='grid grid-cols-1 md:grid-cols-3 gap-4'>
            <div>
              ...
              <Input
                ...
                placeholder={t('staffReturnCar.returnDetails.odometerPlaceholder')}
                aria-invalid={!!odoError}
                className={odoError ? 'border-red-500 focus-visible:ring-red-500' : ''}
              />
        ...
        <Card>
          ...
          <CardContent className='flex items-center justify-between gap-3'>
            ...
            <div className='flex gap-2'>
              ...
              <Button onClick={handleCompleteReturn} disabled={!!odoError}>
