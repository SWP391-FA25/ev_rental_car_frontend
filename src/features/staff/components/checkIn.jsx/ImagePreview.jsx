...
// Accept either `images` (array) with onRemoveImage(id) OR `image` (single) with onRemove()
export function ImagePreview({ images = null, image = null, onRemoveImage, onRemove }) {
    // If single image provided
    if (image) {
        return (
            <div className="group relative overflow-hidden rounded-lg border border-border bg-muted aspect-square">
                <img
            ...
            {images.map((img) => (
                ...
                >
                    ...
                    <button
                        type="button"
                        onClick={() => {
                            if (typeof onRemoveImage === "function") onRemoveImage(img.id)
                        }}
                        className="absolute inset-0 flex items-center justify-center bg-black/50 opacity-0 transition-opacity group-hover:opacity-100 focus:opacity-100 focus:outline-none"
                        aria-label={`Xóa ảnh ${img.category}`}
                    >
                        <X className="h-5 w-5 text-white" />
                    </button>
                </div>
